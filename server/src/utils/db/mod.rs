...
pub async fn get_database_url(kms_client: &Client) -> String {
    ...
    if let Ok(database_url) = env::var("DATABASE_URL") {
        println!("Using DATABASE_URL directly: {}", database_url);
        return database_url;
    }

    println!("Using KMS to decrypt DB_PASSWORD");
    let x = decrypt_kms(
        kms_client,
    ...
    println!(
        "Successfully decrypted password (encoded length: {})",
        db_password.len()
    );
    ...
    println!(
        "Generated database URL (showing host only): postgres://{}@{}/{}",
